...
The lower-level agent harness can also generate compactions that embed retained post-compaction context directly on the entry, instead of `firstKeptEntryId`. This `retainedTail` form is not produced or consumed by coding-agent's `SessionManager`:
...
The agent harness has separate context-building APIs that understand `retainedTail` checkpoints; coding-agent's `buildContextEntries()` and `buildSessionContext()` use `firstKeptEntryId` only.
