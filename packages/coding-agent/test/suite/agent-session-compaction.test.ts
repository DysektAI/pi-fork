...
describe("AgentSession compaction characterization", () => {
	...
	it("reports an AbortError thrown by the summarization stream as a failure", async () => {
		const abortFailedEvents: Array<{
			...
			extensionFactories: [
				(pi) => {
					pi.on("session_compact_failed", async (event) => {
						abortFailedEvents.push(event);
					});
				},
			],
		});
		harnesses.push(abortHarness);
		seedCompactableSession(abortHarness);
		abortHarness.session.agent.streamFunction = () => {
			throw new DOMException("cancelled", "AbortError");
		};
		const abortInternals = abortHarness.session as unknown as SessionWithCompactionInternals;

		await expect(abortInternals._runAutoCompaction("threshold", false)).resolves.toBe(false);

		// Only a real cancellation (aborted controller / extension cancel) counts as `aborted`.
		// An AbortError from the summarization stream is an unrelated failure; upstream's
		// de2de549b regression coverage (9340-9777) pins the same policy for auth failures.
		expect(abortHarness.eventsOfType("compaction_end").at(-1)).toMatchObject({
			type: "compaction_end",
			reason: "threshold",
			aborted: false,
			willRetry: false,
			errorMessage: "Auto-compaction failed: cancelled",
		});
		expect(abortFailedEvents).toEqual([
