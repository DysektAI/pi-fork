...
function generateFireworksModels(
	...
	writeFileSync(
		...
		`const catalog = ${JSON.stringify(catalog)};\n` +
			...
			// Fork patch (feat/synthetic-provider): the Synthetic provider adds one more catalog fetch.
			`  if (url === "https://api.synthetic.new/openai/v1/models") return Response.json({ data: [] });\n` +
